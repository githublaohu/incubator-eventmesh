...
public class HttpRpcContext extends Context {
	...
	protected void init() {
		...
		try {
			if (Objects.equals(this.httpRequest.headers().get(ProtocolKey.PROTOCOL_NEW_VERSION), "true")) {
				...
				// TODO
				Header.buildHeader(identifying, parseHttpHeader(httpRequest));
				Body.buildBody(identifying, bodyMap);
			}
	...
	private HttpResponse oldProtocol() {
		...
		EventMeshRetCode retCode = REQUEST_RET_CODE.get(requestCode);
		...
		body.put("retCode", EventMeshRetCode.SUCCESS.getRetCode());
		...
		if (this.isSuccess) {
			...
			if (Objects.nonNull(this.response.getBodyObject())) {
				...
				body.put("retMsg", EventMeshRetCode.SUCCESS.getErrMsg());
			}

		} else {
			body.put("retMsg", EventMeshRetCode.SUCCESS.getErrMsg() + EventMeshUtil.stackTrace(this.throwable, 2));
		}
